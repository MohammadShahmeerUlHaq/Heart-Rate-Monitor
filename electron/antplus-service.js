...
class AntPlusService extends EventEmitter {
  ...
  async startScanning() {
    ...
    try {
      ...
      this.stick.on('data', (data) => {
        ...
        // ✅ Only log raw events if debugging is enabled
      ...
      this.stick.on('startup', () => {
        ...
        // Set up multiple HR sensor channels (up to 8 for better device support)
        for (let i = 0; i < 8; i++) {
          this.addHRSensor(i);
        }
        
        // Start periodic updates to frontend
      ...
      // Check if stick can be opened before proceeding
  ...
  async stopScanning() {
    ...
    // Wait for all sensors to detach
    ...
    // Close the stick with proper error handling
    if (this.stick) {
      try {
        console.log('Closing ANT+ stick...');
        // Wait a bit more before closing to ensure no pending requests
